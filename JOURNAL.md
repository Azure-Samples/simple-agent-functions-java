...
## 2026-03-18 — Java Port Complete

> **[Coder]** Ported simple-agent-functions from TypeScript to Java.

### What Happened

Ported the entire `simple-agent-functions-typescript` Azure sample to Java:
- **Ask.java**: Azure Functions HTTP trigger using `CopilotClient` from `copilot-sdk-java`, with BYOK support (Azure OpenAI + DefaultAzureCredential)
- **Chat.java**: Interactive console client for testing the agent
- **Infra**: Full Bicep infrastructure copied from TS, updated to `java/17` runtime
- **Build**: Maven project compiles and tests pass (`mvn clean package`)
- Pushed to https://github.com/Azure-Samples/simple-agent-functions-java

### Key Decisions
- Used `copilot-sdk-java:0.1.32-java.0` — the latest stable release
- Session approach: create CopilotClient per request, use `sendAndWait()` for synchronous response
- Chat client uses plain `HttpURLConnection` to keep dependencies minimal

### What's Next

- [ ] End-to-end testing with live Copilot CLI
